...
router.post('/student/google', async (req, res) => {
  try {
    ...
    if (isAdminEmail) {
      ...
      if (mongoose.connection.readyState !== 1) {
        ...
        const jwtToken = jwt.sign(
          { id: mockAdmin._id, userType: 'admin' },
          process.env.JWT_SECRET || 'your-secret-key',
          { expiresIn: '24h' }
        )
    ...
    if (mongoose.connection.readyState !== 1) {
      // Demo mode - create mock student data
      const mockStudent = {
        _id: googleId,
        googleId,
        email,
        name,
        picture,
        studentId: `DEMO${googleId.slice(-4)}`,
        year: '1st Year',
        major: 'Computer Science'
      }

      const jwtToken = jwt.sign(
        { id: mockStudent._id, userType: 'student' },
        process.env.JWT_SECRET || 'your-secret-key',
        { expiresIn: '24h' }
      )
